 ...
 * kgnguyen
 *
 * Contains the methods for class Triangle
 */
...
void Triangle::read(istream& ins) {
    ...
    // try reading as the first format, if fails then
    // read the second format starting from vertexTwo
    ins >> vertexOne >> vertexOneColor
        >> vertexTwo >> vertexTwoColor
        >> vertexThree >> vertexThreeColor;
    if (ins.fail()) {
        ins.clear();
        ins >> vertexTwo >> vertexThree >> vertexColor;
        vertexOneColor = vertexColor;
        vertexTwoColor = vertexColor;
        vertexThreeColor = vertexColor;
    }
